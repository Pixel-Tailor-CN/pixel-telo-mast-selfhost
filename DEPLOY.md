...
`init` 默认监听 `0.0.0.0:8443`、`tls.mode=auto`。未传 `--public-url` 时：终端里会列出本机网卡地址并可选查询公网 IP；非交互环境会打印候选地址后失败。只想本机访问时加 `--listen 127.0.0.1:8443`。容器内列出的地址通常不是手机能访问的宿主机 IP。前置反代使用 `--tls-mode off --allow-insecure-private-network`。`serve` 仅在监听 `0.0.0.0` / `::` 时于控制台列出已配置 URL 和本机检测到的其它候选地址。
...
- `proxy_url`：可选的固定 HTTP/HTTPS 代理，例如 `http://127.0.0.1:8080`，仅作用于对应 Provider。支持 URL 用户名密码，不支持 SOCKS、轮换或自动重试。保持 TLS 校验，连接代理也计入默认 2 秒超时；未配置时默认直连，`HTTPS_PROXY` 等通用环境变量不会生效。

搜狗返回验证码、403/429 或已知验证码重定向时会立即冷却：有有效 `Retry-After` 时遵守该值，否则 30 秒。未识别页面或号码不匹配返回上游不可用，不会返回非骚扰。可按实际访问量调低并发并设置 `min_interval`，但等待仍计入总超时；固定代理不保证能消除反爬。代理凭据按秘密管理，不提交到 Git。
...
| `MAST_SOGOU_PROXY_URL` | 可选。搜狗固定 HTTP/HTTPS 根代理 URL，如 `http://proxy.example:8080`；不填写则直连。 |
| `MAST_360_PROXY_URL` | 可选。360 固定代理 URL；规则同上。 |

这两个代理变量不会隐式启用 Provider。Vercel 的并发限制、请求间隔和反爬冷却属于单实例内存状态，多个函数实例不会共享冷却。配置代理后仍需用有效号码卡片成功率判断效果，不能只看 HTTP 200。
