# Mast Self-host 自建部署手册

本文面向熟悉 Linux、容器、DNS、TLS 和服务运维的高级用户，说明如何独立部署 Pixel Telo Mast Self-host。本文不提供反馈、管理、Prometheus 或官方实时查询代理能力。
...
3. 启用哪些网页 Provider，并确认其条款、访问政策和许可。
4. 数据目录、备份目录和运行账户。
5. 固定使用的镜像或二进制版本。
...
~~~bash
export MAST_IMAGE=mystery0/pixel-telo-mast-selfhost:v0.1.2
...
init 只生成 config.yaml，不生成 Token、Runtime 数据库、证书或 Instance ID。启动前必须检查并编辑配置。
...
生产环境应使用服务管理器负责自动启动、异常重启、SIGTERM 优雅关闭和日志收集。不要以 root 身份长期运行。
...
高级用户应使用支持自动续期和部署钩子的 ACME 客户端。签发结果至少包括：
...
upstream.provider_ids 决定允许查询的 Provider 及默认优先级。列表顺序就是优先级顺序。

providers 下的参数含义：

- min_interval：同一 Provider 两次实际请求开始的最小间隔。
- max_concurrent：同一 Provider 最大并发。
- breaker_timeout：连续失败达到阈值后的熔断时间。

提高实例限流不代表上游允许同等请求速率。公开服务应从保守参数开始，根据 429、Retry-After、超时和上游政策逐步调整。

默认查询总超时为 2 秒，最大允许配置为 10 秒。公开功能验证实例不建议为了掩盖上游不稳定而盲目提高超时。
...
~~~bash
curl --fail --show-error https://mast.example.com:8443/api/health
~~~
...
配对输出包含 URL、Token、Instance ID 和可能存在的 SPKI Pin。该输出必须按密码处理，不得进入日志、工单或公开剪贴板历史。
...
除 /api/health 外，所有接口都必须携带 Bearer Token。
...
显式设置 tls.public_url。前置 TLS 模式必须填写客户端实际访问的 HTTPS 根 URL，不能使用后端 HTTP 地址。
